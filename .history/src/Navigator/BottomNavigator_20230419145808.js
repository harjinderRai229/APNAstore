          ...
          return (
            <View style={[styles.tabIconWrapper, { backgroundColor }]}>
              ...
              {tab.name === 'CartTab' && <View style={styles.circle} >
                <Text style={{ color: '#fff', fontWeight: '700' , fontSize:12}}>{data.reducers1.length}</Text>
              </View>}
              {tab.name === 'WishListTab' && <View style={[styles.circle, { height: 18, width: 18, borderRadius: 9 }]} >
                <Text style={{ color: '#fff', fontWeight: '700' }}>{data.reducers2.length}</Text>
              </View>}
            </View>
          );
        },
